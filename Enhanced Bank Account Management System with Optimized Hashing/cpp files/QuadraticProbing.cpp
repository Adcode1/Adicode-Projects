...
void QuadraticProbing::createAccount(std::string id, int count)
{
    ...
    else
    {
        if ((bankStorage1d[key].id != ""))
        {
            ...
            for (int i = 0; i < s; i++)
            {
                if (bankStorage1d[(key + i * i) % s].id == "")
                {
                    bankStorage1d[(key + i * i) % s] = a;
                    break;
                }
            }
        }
...
void mergeQ(std::vector<int>& arr, int left, int mid, int right) {
    int n1 = mid - left + 1;
    int n2 = right - mid;
...
void mSortQ(std::vector<int>& arr, int left, int right) {
    if (left < right) {
        int mid = left + (right - left) / 2;
        mSortQ(arr, left, mid);
        mSortQ(arr, mid + 1, right);

        mergeQ(arr, left, mid, right);
    }
}
